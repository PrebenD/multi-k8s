...
docker build -t prebenmd/multi-server-k8s:latest -t prebenmd/multi-server-k8s:$SHA -f ./server/Dockerfile ./server
...
docker push prebenmd/multi-server-k8s:latest
docker push prebenmd/multi-worker-k8s:latest

docker push prebenmd/multi-client-k8s:$SHA
docker push prebenmd/multi-server-k8s:$SHA
docker push prebenmd/multi-worker-k8s:$SHA

kubectl apply -f k8s
kubectl set image deployments/server-deployment server=prebenmd/multi-server-k8s:$SHA
